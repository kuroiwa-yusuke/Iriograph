...
test("English is the default and changing the UI language does not modify the document", async ({ page }) => {
  ...
  await expect(page.locator(".iriograph-node-label", { hasText: "Choose a pizza" })).toBeVisible();
  await expect(page.locator(".iriograph-edge-label", { hasText: "Next step" }).first()).toBeVisible();
  ...
  await expect(page.locator(".iriograph-node-label", { hasText: "ピザを選ぶ" })).toBeVisible();
  await expect(page.locator(".iriograph-edge-label", { hasText: "次の工程" }).first()).toBeVisible();
  await expect(page.locator(".iriograph-node-label", { hasText: "Choose a pizza" })).toHaveCount(0);
