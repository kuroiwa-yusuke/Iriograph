...
旧Pizza単体の試行とは別に、Pizza注文、購買承認、service architectureの3画像を固定し、medium 1回、高推論2回の各3runで再実験しました。公開する`docs/experiments/references`には購買承認とservice architectureだけを置き、Pizza画像は除外します。以下のPizza値は当時の実験履歴であり、配布する再構成入力ではありません。既存`.iriograph`、Turtle、overlay、SVG原本は画像再構成agentへ見せていません。
