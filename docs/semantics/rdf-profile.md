...
| `literal-annotation` | literal値を持つstatement | provenance付き`annotation` |
...
catalogのprojection部分は、次の正規化された`rules`を正本とします。旧`nodeRules`、`relationRules`、`containmentRules`は既存host移行用の互換入力に限定します。
...
Domain resourceとvocabulary、appearance-only情報の判定は[意味と表示notation](./notation.md)に従います。
