...
`auto`のroute family選択は、障害物を横切らず既存候補よりedge交差・重複を悪化させない直線、水平・垂直の二線分と直角一回だけで成立する最大一中間点の直交折線、内角90度以上のcorridorをBezier smoothingした緩い曲線、それ以外の最大一中間点折れ線の辞書順です。直交候補は曲線より単純で読みやすい特殊形として先に扱い、任意角度の折れ線を曲線より先にはしません。曲線はcontrol polygonではなくflattenした実Bezierでnode、常時予約されるcomment領域、他edgeとの交差・重複、endpointからの外向き、parallel laneの同一化を評価します。一条件でも悪化すれば次候補へ進みます。`straight`、`curve`、`orthogonal`、`manual`の明示指定はfamilyを変更せず、`auto`だけを選択対象にします。
...
0.9.0のピザ注文・配送fixtureでは、同一ProjectedSceneを固定1920×1080で比較し、標準adapterを採用しました。Optional ELKの既存比較値はnode overlap 0、edge交差6、route-node交差3、fit 24%、aspect 3.18、lane順不成立でした。現在の標準adapterはnode overlap 0、route-node交差1、共有endpointを除くedge交差7、edge overlap 0、fit 47%、aspect 1.8029、lane順成立です。自動route familyの優先変更後も構造評価91.1/100を維持しています。この結果はadapter選択の検証証拠であり、fixtureのIRIやlabelを検出するruntime分岐には使いません。
