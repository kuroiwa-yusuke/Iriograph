...
| P1-56 | Label typography overlay | sparse appearance overlay、text measurement | Node、包含領域・順序付き・候補の各group frame、edge caption/commentごとにcatalog既定を基準とするboundedなfont sizeをビューInspectorから変更できる。Group labelは既定でも判読可能な大きさとし、文字方向・anchor・offsetとは独立させる。保存するのは既定との差分だけで、text boundsを衝突・包含検査へ反映し、表示変更だけで全体layoutを再実行しない |
| P1-57 | 所属一覧の直接編集と発見性 | exact membership provenance、P2-11 | 要素詳細の「属するグループ」と包含領域・順序付きグループ・候補グループ詳細の「含む要素」から、通常membershipを個別・複数解除し、対象をCanvasへfocusし、所属editorへ移れる。既存のexact provenanceとatomic解除を再利用し、Seq/Altだけはordinal/default slotを壊さない専用editorへ送る。MockとCloudの双方で同じ操作が選択直後に見つかる |
...
| P1-59 | Source tab再編と全文Document editor | semantic/overlay transaction、runtime schema、all-view validation | Document内のread-only Turtle全文を廃止してTurtle tabへの導線と意味要約に置き換える。Overlay sourceとportable document全体をともに通常表示するeditable JSONとし、全文をそのままcopy/pasteできる。全文適用はparse、schema、Turtle/semantic/profile、全named view projection・空間制約、stale draftを検証する一つのatomic replace transactionとし、partial commitせず、一回のundo/dirty/save flushとJSON Pointer付き日本語errorを返す。技術的な内容であってもAdvancedへ隔離せず、利用しないユーザーはsectionを閉じられる |
...
| P1-61 | 順序付きグループ・候補グループの通常authoringと統一UI | structured authoring、resource allocator、Seq/Alt projection | 順序付き・候補グループ本体は`要素を追加`で種類を明示して先に作り、作成直後から空のGroup Frameとして選択できる。`関係を追加`は既存groupへのmember追加・解除だけを行い、通常要素をgroupへ変換したり構造resourceを暗黙生成したりしない。Seq/Alt、`rdf:_n`、IRIを露出せず、両構造はCanvas複数選択、共通のordered card list、drag/上下移動、追加・除外を共有し、候補グループだけ既定項目を指定する。順序付きグループは薄い枠、header、番号badgeに加え、明示保存された隣接ordinal間を矢印・labelなしの低彩度なderived順序ガイドで結ぶ。通常edge renderer/routingを再利用しても、処理遷移を主張するdirect predicate triple、terminal、個別route overlayを生成しない。候補グループは薄い構造枠とheader badgeを持ち、候補hubから各memberへ矢印・predicate labelなしの低彩度なderived候補線を描く。両derived線は明示ordinal statementだけから作り、geometry、label、既存edgeから順番・候補を推論しない。Hover、ARIA、hit test、選択概要でdirect relationと区別し、選択時は線単体でなく対応構造を選んで共通editorを開く。空groupは作成途中として保存可能にし、順序付き1件未満、候補2件未満、候補の既定未設定は壊れたTurtleでなく「内容が未完了」の修正行動付きdiagnosticとして扱う。重複、構造だけの削除、member削除の波及、再採番、Mock/Cloudのcomponent/E2Eを確認する |
...
| P1-64 | View Inspectorの情報設計簡素化 | appearance editor | 「ビューを編集」ボタンを挟まず、選択直後から対象別の小さいsectionを表示する。Nodeは形・style／icon・内容／位置・size、包含領域・順序付き・候補の各group frameは枠style／名称・層／位置・size、edgeは線形式／接続点・terminal／label・補足／線styleへ分け、各sectionを折り畳める。利用不能field、source/target数値、重複する補助表示を出さず、変更は既存の即時一履歴規則を維持する |
| P1-65 | 対象別context menu | P1-57、P1-61、P1-64、accessibility | 右click、ContextMenu key、Shift+F10から同じ対象別menuを開き、nodeは詳細・ビュー・icon・関係作成・所属・削除、direct edgeは関係詳細・線・再接続・route reset・削除、derived順序ガイド・候補線は対応する構造編集、包含領域・順序付きグループ・候補グループはmember/順序・枠・fit・許可層内z-order・削除、空白は要素追加とpasteを入口として提示する。Menu選択自体では変更せず該当Inspector/actionへfocusし、raw IRIや一段で意味を変える危険操作を置かない。既存の削除確認条件、keyboard focus return、disabled理由を維持する |
...
| P1-67 | 共通Group Frameのビュー編集契約 | spatial membership、P1-56、P1-61、P1-64 | `rdf:Bag`やclass membershipの包含領域、`rdf:Seq`の順序付きグループ、`rdf:Alt`の候補グループを、semantic kindとexact provenanceを失わない共通Group Frameとして描画・選択する。位置、size、drag、8 handle resize、要素に合わせる、背景色、枠色・線種・太さ、透明度、label anchor・方向・font size、構造層内z-order、resetを同じビューInspectorとsparse overlay契約で扱う。全member boundsとpaddingを枠内に保ち、複数group所属では全content intersection、group同士の重なり、選択中も`group < edge < node`を共通制約にする。意味編集だけは包含の追加・解除、Seqの順番、Altの候補・既定へ分け、geometryからmembership、順番、候補を推論しない。Overlay-onlyの枠変更では全体layoutを起動せず、対象frameとderived connector・incident edgeだけを局所更新し、Mock/Cloudのcomponent/E2Eで三種類の操作同等性を固定する |
| P1-68 | Iconで選ぶ関係・所属palette | P1-61、P1-67、predicate catalog、structured authoring | `関係を追加`の最初は文字一覧を展開せず、投影後の代表形を略図化した大きなicon cardで「線でつなぐ」と「グループへ所属させる」の二つだけを示す。これは個別styleを決める見た目編集ではなく、direct edgeか既存group membershipかというつなぎ方の選択とする。前者はcategory別predicate一覧、一つの始点、一件以上の接続先を一画面に段階表示し、一始点から複数接続先は共通predicateを既定、必要時だけ行別predicateを展開して一つのatomic transactionへ確定し、暗黙の多対多edgeを生成しない。後者は分類・包含・順序付き・候補をiconと説明でgroupingした既存Group Frame一覧から一件をCanvasで選び、member候補を複数選択する。順序付きでは並べ替え、候補では既定指定を同じ段階へ追加するが、新しいgroupの作成、通常要素のgroup化、group kind変換は行わない。Groupがない場合は`要素を追加 → グループ`への導線を示し、関係draft内で暗黙作成しない。既存Canvas選択は各役割をlabelで明示してからfamily別draftへseedし、family切替で相互変換しない。戻る、Escape/Cancel、stale revision、別Preview/Applyなしのatomic history、icon以外の名称・説明、選択件数通知、keyboard/focus、無効理由を備える。Mock/Cloudの狭幅を含むcomponent/E2Eで事前選択0・1・複数、通常nodeをgroupとして選べないこと、戻る、validation失敗時の無変更、複数edge・membershipの部分失敗rollbackを固定する |
| P1-69 | 要素作成時の種類・分類選択とGroup Frame生成 | authoring profile、classification-region、resource/term allocator、P1-67 | `要素を追加`の最初は形のpreview付きで「要素」と「グループ」を選ぶ。「要素」は名前に加え、resolved profileが許可するnode-roleの既存classをlabel・説明・previewから一件以上選び、opaque IRI、label、全`rdf:type`を一つのatomic transactionで作る。複数classを許容するが、同一resourceへnodeとgroupのstructural roleを混在させない。Canvasの分類領域またはderived交差領域を明示pickerで選んだ場合は構成classをdraftへseedするが、作成位置や単なる重なりから分類を推論しない。分類候補がある標準UIでは最低一件を要求し、profileが未分類resourceを明示許可する場合だけ「未分類で作る」を出す。「グループ」はicon cardから「分類グループ」「包含グループ」「順序付きグループ」「候補グループ」を選び、前者は名前と`rdfs:Class`、後三者は名前と`rdf:Bag`、`rdf:Seq`、`rdf:Alt`をそれぞれ一つのatomic transactionで作る。通常UIにはclass、container type、IRIを出さず、分類は種類による集合、包含は順序なし、順序付き、候補という違いを説明する。Active region viewでは作成直後から選択済みの空Group Frameとして表示し、対応しないviewでは見えないresourceを作る前に「領域で表示できるビューを開く」導線を示す。`要素を変更`ではclassの追加・解除と明示的なstructural role変更を扱えるが、`関係を追加`からは変更できない。互換な種類変更は直接確定し、既存member・ordinal・候補を失う変更は黙って削除せず、先に所属を解除する修正行動を示して拒否する。Bag/Seq/Alt間の構造変換は単純なclass変更に含めない。Resolved capability、権限とallocator、空group、分類0・1・複数、交差領域seed、role競合、種類変更、validation失敗、undo/redoをMock/Cloudで検証する |
| P1-70 | Group所属中の新規member同時作成 | P1-68、P1-69、resource allocator、atomic batch authoring | P1-68のmember一覧に`新しい要素を追加`を置き、既存の詳細formを入れ子にせず、P1-69と同じ名前・node-role class選択だけをcompactな一段階で行う。追加中の要素はdocument外のephemeral chipとしてCanvas選択済みmemberと同じ一覧に並べ、確定時に各opaque IRI、各label・`rdf:type`と、既存groupへの全membership・ordinal・既定候補を一つのcandidate patchへまとめる。新しいgroupやgroup-role要素はここから作れない。一件でもallocation、profile/domain validation、全view projectionに失敗したら新規要素とmembershipを一件も保存せず、成功時だけ一semantic historyとして確定する。順序付きでは作成前に並べ替え、候補では既定指定ができ、名前の重複はidentityとして統合しない。Cancel、戻る、stale revision、複数新規要素、既存memberとの混在、class未選択、部分失敗rollback、完了後のfocusをMock/Cloudで検証する |
