...
| P1-61 | 順序付きグループ・選択肢の通常authoringと統一UI | structured authoring、resource allocator、Seq/Alt projection | 初期入口は`要素を追加`と`つながりを追加`の二つに留め、後者で「関係：AとBの意味関係」「順序付きグループ：複数要素の順番」「選択肢：複数候補から選ぶまとまり」の3 cardを日本語説明付きで示す。Seq/Alt、`rdf:_n`、IRIを露出しない。後二者は名前、Canvas複数選択、共通のordered card list、drag/上下移動、追加・除外を共有し、選択肢だけ既定項目を指定する。Allocatorで構造resourceを発行し、type、label、全ordinalを一回のatomic create/editへ確定する。順序付きグループは薄い枠、header、番号badgeに加え、隣接ordinal間を矢印・labelなしの低彩度なderived順序ガイドで結ぶ。通常edge renderer/routingを再利用しても、処理遷移を主張するdirect predicate triple、terminal、個別route overlayを生成しない。選択肢は選択nodeからmemberへの無名derived branchを同じrendererで描く。両derived線はhover、ARIA、選択概要でdirect relationと区別し、選択時は線単体でなく対応構造を選んで共通editorを開く。固有predicateや「承認時」等の枝ごとの意味が必要な業務分岐は通常edgeを使う。Seq 1件以上、Alt 2件以上、既定、重複、構造だけの削除、member削除の波及を検証し、Canvas位置から順番・選択を推測せず、Mock/Cloudのcomponent/E2Eで同じ操作を確認する |
...
| P1-65 | 対象別context menu | P1-57、P1-61、P1-64、accessibility | 右click、ContextMenu key、Shift+F10から同じ対象別menuを開き、nodeは詳細・ビュー・icon・関係作成・所属・削除、direct edgeは関係詳細・線・再接続・route reset・削除、derived順序ガイド・分岐線は対応する構造編集、region/Seqはmember/順序・枠・fit・許可層内z-order・削除、空白は要素追加とpasteを入口として提示する。Menu選択自体では変更せず該当Inspector/actionへfocusし、raw IRIや一段で意味を変える危険操作を置かない。既存の削除確認条件、keyboard focus return、disabled理由を維持する |
