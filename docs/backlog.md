...
| P1-61 | 順序付きグループ・選択肢の通常authoringと統一UI | structured authoring、resource allocator、Seq/Alt projection、P1-65 | 初期入口は`要素を追加`と`つながりを追加`の二つに留め、後者で「関係：AとBの意味関係」「順序付きグループ：複数要素の順番」「選択肢：複数候補から選ぶまとまり」の3 cardを日本語説明付きで示す。Seq/Alt、`rdf:_n`、IRIを露出しない。後二者は名前、Canvas複数選択、共通のordered card list、drag/上下移動、追加・除外を共有し、選択肢だけ既定項目を指定する。Allocatorで構造resourceを発行し、type、label、全ordinalを一回のatomic create/editへ確定する。順序付きグループは薄い枠、header、番号badgeのままmember間の偽edgeを作らない。選択肢は選択nodeからmemberへの無名derived branchを通常edge renderer/routingで描くが、hover、ARIA、選択概要でdirect relationと区別し、選択時はpredicate editorでなく共通の構造editorを開く。固有predicateや「承認時」等の枝ごとの意味が必要な業務分岐は通常edgeを使う。Seq 1件以上、Alt 2件以上、既定、重複、構造だけの削除、member削除の波及を検証し、Canvas位置から順番・選択を推測せず、Mock/Cloudのcomponent/E2Eで同じ操作を確認する |
