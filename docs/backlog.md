...
| P1-43 | Curve edgeの実Bezier描画とrich control | P1-39、routing overlay schema、SVG hit testing | `curve`をstraight/polylineとの重複描画で代用せず、endpoint、on-curve knot、Bezier control pointから連続した実curveとして描画する。CanvasとビューInspectorからcontrol point/knotの追加・削除、handle dragによる角度・曲率変更、最後のmanual control削除によるautomatic curve復帰を行え、各gestureを一つのundo itemにする。Curve固有controlはsparse view overlayだけへ保存し、S/P/OとTurtle、node geometry、他edgeのrouteを変更しない。0/1/複数knot、self-loop、parallel edge、zoom別hit area、keyboard操作、保存往復をcomponent/E2Eで再現し、選択時もcurveとstraightを二重表示しない |
| P1-44 | 選択要素の包含一覧 | P1-41、Scene membership provenance、semantic-access membership index | Resource、region、container、Seqを選択した意味detailsに、「この要素が属する領域」と「この要素が含む要素」をlabel-firstで一望できる一覧として出す。空、nested、多対多、100件以上を含むfixtureで件数、方向、所属種別、ordinalを失わず、一覧から対象をfocus/revealできる。Geometry上の内外から包含を推測せずTurtleのexact membershipとScene provenanceを読むだけとし、一覧自体をoverlayへ複製しない。追加・解除は既存semantic transaction、一覧の展開・選択はsession stateとして分離する |
| P1-45 | Diamond nodeのlabel方向・幅回帰 | P1-38、template content bounds、node label renderer | Diamond templateで横書きlabelが不必要に一文字幅へ潰れる、または縦書き方向が反映されない状態を修正する。横書き/縦書き、短文/長い日本語、icon有無、resize前後の組合せで、diamondの安全な内接content boundsを使って折返し・clip・offsetを決定し、labelがnode外へ消えず選択handleを妨げないことをscreenshot regressionで確認する。文字方向・offset・個別sizeはview overlay、label本文はTurtleという境界を維持し、layoutはtemplateの実占有boundsだけを参照する |
| P1-46 | 関係transactionの低遅延化とlayout安定性 | semantic prepared pipeline、display reconciliation、P1-39、P1-42 | 現行local Cloudのpizza workspaceはfile clickから25 node/32 edgeの描画完了まで1.91〜1.97秒かかり、files API 8ms、document GET 4ms、font 16msに対して約1.6秒をparse/projection/layout/routeのmain-thread CPUが占める。このfixtureでparse、projection、placement、初期route、refinement、compaction、validation、asset、paintのphase別時間、発火回数、long taskを計測し、document body受領からsettled Sceneまでp95 300ms以下、20回warm後の既存node間relation追加・predicate変更・endpoint変更は操作開始からsettled Sceneまでp95 150ms以下にする。Preview、apply、publishで同じprevious/candidate Sceneを再構築せず、revision・command・confirmation fingerprintに束縛したprepared resultを再利用し、一回のrelation確定に対するcandidate layoutを最大1回、publish時の追加layoutを0回とする。可視node/region/container集合、membership、Seq/Alt ordinalが不変なedge-only transactionではfull placementを実行せず、存続するmanual/user overlay、pinned/geometry/appearance/manual route、包含boundsをbyte-equivalentに維持し、追加/変更edge、endpointのincident route、影響connected componentのrouteだけを更新する。Full layout fallbackは可視primitive集合、membership/ordinal、layoutRef/direction、generated geometryの互換性のいずれかが変わる場合に限定し、理由をdiagnostic/計測へ残す。既存node/region移動0、membership warning増加0を必須にする。500 node等でfast pathだけを測らず、pizza、疎なsmall graph、密なsmall graphを性能fixtureに含める。同一表示寸法の無関係なlabelへ全置換したfixture、code-point順を保ったopaque IRIへ写像した同型fixture、構造の異なる非pizza包含fixtureで正規化後の安定性とroute品質を比較し、core/layoutに`pizza` namespace・label・node/edge件数・特定topologyによる分岐がないことをtestとsource reviewで確認する。RelationのS/P/OだけをTurtle正本、存続user調整をoverlay正本、局所route completionと明示fallbackをlayout責務とする |
| P1-47 | 関係追加の終点選択分離 | P1-41、Canvas resource picker、authoring validation | 始点選択後に終点pickerを開いてもprimary selection中の始点を自動採用せず、利用者が別nodeを明示選択するまで次段階へ進めない。始点click、blank/region click、Escape、keyboard navigation、Scene更新を再現し、誤って`S === O`のrelationを確定せずfield近傍へ次の操作を示す。Profileがself relationを許可する場合だけ専用の明示actionから作れ、通常の2個目選択とは分ける。未実行選択はsession draft、確定relationだけをTurtleへ保存し、overlay/layoutを選択状態の正本にしない |
| P1-48 | Layout方向のview設定 | layout adapter direction、ViewCommand、generated/user placement | 新規viewと標準業務viewはhorizontal（LR）を既定とし、ビュー設定からvertical（TB）へ切り替えられる。方向はnamed viewのlayout設定として保存しTurtleやelement appearance overlayへ混ぜない。切替時はgenerated geometryとderived routeだけを対象layout adapterで再計算し、user/pinned geometry、全membership、semantic sourceを維持する。Standard/ELK双方で同じfixtureをLRでは主flowのX単調、TBではY単調に配置し、undo/redo、reload、複数view独立性、未知directionのfail-closedをcomponent/E2Eで確認する |
...
P0と現在のP1を満たした状態を最初の実用MVPとします。表に残るP1は未実装または最終検証待ちであり、MVP完了とは判定しません。P2はcloud/LLM/運用の次段階としてcore/editorのMVP判定と分けます。P1項目が完了したら表へ完了行を残さず、基準点と規範文書へ結果を統合します。
