...
| P1-61 | 順序付きグループ・候補グループの通常authoringと統一UI | structured authoring、resource allocator、Seq/Alt projection | 初期入口は`要素を追加`と`つながりを追加`の二つに留め、後者で「関係：AとBの意味関係」「順序付きグループ：複数要素の順番」「候補グループ：同じ役割を持つ候補のまとまり」の3 cardを日本語説明付きで示す。Seq/Alt、`rdf:_n`、IRIを露出しない。関係は一つの始点から接続先を一件または複数選べ、各行でpredicateと説明を指定したdirect edge群を一回のatomic transactionで確定し、並列・条件・イベント等の一般的な分岐にSeq/Altを暗黙生成しない。後二構造は名前、Canvas複数選択、共通のordered card list、drag/上下移動、追加・除外を共有し、候補グループだけ既定項目を指定する。Allocatorで構造resourceを発行し、type、label、全ordinalを一回のatomic create/editへ確定する。順序付きグループは薄い枠、header、番号badgeに加え、明示保存された隣接ordinal間を矢印・labelなしの低彩度なderived順序ガイドで結ぶ。通常edge renderer/routingを再利用しても、処理遷移を主張するdirect predicate triple、terminal、個別route overlayを生成しない。候補グループは薄い構造枠とheader badgeを持ち、候補hubから各memberへ矢印・predicate labelなしの低彩度なderived候補線を描く。両derived線は明示ordinal statementだけから作り、geometry、label、既存edgeから順番・候補を推論しない。Hover、ARIA、hit test、選択概要でdirect relationと区別し、選択時は線単体でなく対応構造を選んで共通editorを開く。Seq 1件以上、Alt 2件以上、既定、重複、構造だけの削除、member削除の波及を検証し、Mock/Cloudのcomponent/E2Eで同じ操作を確認する |
