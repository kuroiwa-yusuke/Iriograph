...
UI localization and semantic literal selection are separate contracts. `semanticLocales` controls which existing `rdfs:label`, `rdfs:comment`, and catalog lexical forms are preferred. When it is omitted, it follows `uiLocale`; a host may provide a different ordered list. Core accepts the same ordered list as transient `ProjectionOptions.semanticLocales`: a non-empty list takes precedence over the portable `view.locale` for that projection only, while an absent or empty list preserves the saved view behavior. Changing either preference never translates, adds a language tag to, or rewrites an RDF literal, named view, overlay, history, or dirty state. Semantic selection remains deterministic and falls back through the requested languages, untagged values, and the remaining available values.
