...
Package UIの既定は英語で、日本語を選択できます。UI言語はHost/session stateでありTurtle、named view、display overlayを変更しません。既存RDF label/commentをCanvasとEditor metadataへtransientに選ぶ意味言語の優先順は別に指定できます。
