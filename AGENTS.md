...
- 通常の意味編集UIではIRI、`rdf:type`、`rdfs:label`、`rdfs:comment`という語を露出せず、それぞれ種類、名前、説明として扱う。関係候補はcatalog/profileを正本としてsubjectをA、objectをBとしたlocale別例文でcategory別に表示し、英語を既定fallbackとしてopaque option IDからexact termへ解決する。生のIRIは通常UI、tooltip、Advanced DOMへ渡さず、A/Bは候補説明だけに使い確定edgeへ保存しない。
...
- Membership追加では変更前後のexact membershipを比較し、新たに空間制約を受けるmemberとそのsubtreeだけを共通layout completionで局所移動する。存続する外側／内側Groupと既存memberを全体再配置せず、固定Groupに空きがなければ既存図を崩して解決しない。この差分はtransaction-localでありoverlayやTurtleへ補助情報として保存しない。
...
7. 公開packageをlockstep versionへ更新し、tarball consumer検証後にnpmjsの`@iriograph` organizationへpublic publishする。GitHub Actions Trusted PublishingのOIDCを使い、長期tokenやAWS CodeArtifactを公開経路へ使わない。公開先は`https://registry.npmjs.org/`へ固定し、`core`、`rdf-io`、`profile-resolver`、`semantic-access`、`layout-elk`、`profile-kit`、`presentation-tools`、`host-conformance`、`agent-bridge`、`vue-editor`の依存順でexact versionを確認する。既公開versionは変更せずskipし、未公開packageだけをpublishする。認証・network・不正応答を未公開扱いにせず、上書きや別registryへのfallbackを行わない。
